...
class CountryParser(object):
    ...
    def __write_country_to_db(self, line):
        Country.objects.create(
            id=line[self.COUNTRY_FIELDS['geoname_id']],
            name=line[self.COUNTRY_FIELDS['name']].strip(),
            capital=line[self.COUNTRY_FIELDS['capital']].strip(),
            iso=line[self.COUNTRY_FIELDS['iso']].strip(),
            iso3=line[self.COUNTRY_FIELDS['iso3']].strip(),
            iso_numeric=line[self.COUNTRY_FIELDS['iso_numeric']].strip(),
            fips=line[self.COUNTRY_FIELDS['fips']].strip(),
        )

    def __write_aternate_name_to_db(self, line, country_geoname_id_list):
        if line and not line[0].startswith('#'):
            iso_language = line[self.ALTERNATE_COUNTRY_FIELDS['iso_language']]
            geoname_id = int(line[self.ALTERNATE_COUNTRY_FIELDS['geoname_id']])

            if geoname_id in country_geoname_id_list and len(iso_language) in [2, 3]:
                CountryAlternate.objects.create(
                    country_id=geoname_id,
                    name=line[self.ALTERNATE_COUNTRY_FIELDS['name']].strip(),
        ...
        try:
            with open(COUNTRIES_FILE_PATH) as country_file:
                for line in csv.reader(country_file, dialect='excel-tab'):
                    if line and not line[0].startswith('#'):
                        self.__write_country_to_db(line)
    ...
    def get_alternate_names(self):
        ...
        try:
            with open(ALTERNATE_NAMES_FILE_PATH) as country_file:
                for line in csv.reader(country_file, dialect='excel-tab'):
                    self.__write_aternate_name_to_db(line, country_geoname_id_list)
        except Exception as error:
            logger.debug(error)

    def get_alternate_locale_names(self, locale='ru'):
