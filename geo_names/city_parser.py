        ...
        try:
            with open(ALL_COUNTRIES_FILE_PATH) as country_file:
                for line in csv.reader(country_file, dialect='excel-tab'):
                    if line and not line[self.CITY_FIELDS['geoname_id']].startswith('#'):
                        parse_condition = line[self.CITY_FIELDS['feature_class']] == CITY_FEATURE_CLASS
                        ...
                        if parse_condition:
                            ...
                            city_list.append(City(
                                ...
                                country_id=country_id_dict[line[self.CITY_FIELDS['counrty']]],
                                name=line[self.CITY_FIELDS['name']].strip(),
                                latitude=line[self.CITY_FIELDS['latitude']].strip(),
                                longitude=line[self.CITY_FIELDS['longitude']].strip(),
                                timezone=line[self.CITY_FIELDS['timezone']],
                                feature_class=line[self.CITY_FIELDS['feature_class']].strip(),
                                feature_code=line[self.CITY_FIELDS['feature_code']].strip(),
                                date_modification=line[self.CITY_FIELDS['date_modification']].strip(),
                            ))
